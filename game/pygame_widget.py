...
class PygameWidget(QWidget):
    ...
    def init_weather(self):
        """Initialize the foreground atmosphere for every world style.

        Every style used by MazePaint gets a real weather/atmosphere layer.
        The old implementation only handled a small subset of styles, which
        made styles such as leaves, bubbles, stars, petals, scan and aurora
        appear to have no weather effect.
        """
        ...
        self.weather_timer = 0.0
        self.lightning_flash = 0.0
        self.weather_phase = random.uniform(0, math.pi * 2)

        # All supported world styles.
        particle_styles = {
            "leaves", "bubbles", "stars", "embers", "sand", "snow",
            "petals", "scan", "horizon", "sparkles", "rain", "aurora",
            "pulse"
        }

        if style not in particle_styles:
            self.weather_type = None
            return

        self.weather_type = style
        ...
        if style == "rain":
            for _ in range(115):
                self.weather_particles.append({
                    ...
                    "speed": random.uniform(11, 20),
                    "len": random.uniform(10, 23),
            ...
            for _ in range(80):
                self.weather_particles.append({
                    ...
                    "sway_speed": random.uniform(0.01, 0.035),
            ...
            for _ in range(55):
                self.weather_particles.append({
                    ...
                    "sway": random.uniform(0, math.pi * 2),
                    "sway_speed": random.uniform(0.02, 0.055),
            ...
            for _ in range(42):
                self.weather_particles.append({
                    ...
                    "sway": random.uniform(0, math.pi * 2),
                    "sway_speed": random.uniform(0.018, 0.045),
            ...
            for _ in range(45):
                self.weather_particles.append({
                    ...
                    "sway": random.uniform(0, math.pi * 2),
                    "sway_speed": random.uniform(0.015, 0.04),
    ...
    def update_weather(self):
        ...
        if self.weather_type == "rain":
            ...
            if self.weather_timer <= 0 and random.random() < 0.008:
                self.lightning_flash = 1.0
                self.weather_timer = 110
            ...
            for leaf in self.weather_particles:
                leaf["y"] += leaf["speed"]
                leaf["x"] += math.sin(leaf["sway"]) * 0.9
                leaf["sway"] += leaf["sway_speed"]

                if leaf["y"] > height + 20:
                    leaf["y"] = random.uniform(-50, -5)
                    leaf["x"] = random.uniform(-20, width + 20)

        elif self.weather_type == "bubbles":
            for bubble in self.weather_particles:
                bubble["y"] -= bubble["speed"]
                bubble["sway"] += bubble["sway_speed"]

                if bubble["y"] < -15:
                    bubble["y"] = height + random.uniform(5, 40)
                    bubble["x"] = random.uniform(0, width)

        elif self.weather_type == "petals":
            for petal in self.weather_particles:
                petal["y"] += petal["speed"]
                petal["x"] += math.sin(petal["sway"]) * 0.8
                petal["sway"] += petal["sway_speed"]

                if petal["y"] > height + 15:
                    petal["y"] = random.uniform(-30, -5)
                    petal["x"] = random.uniform(-20, width + 20)

        elif self.weather_type in ("stars", "sparkles"):
            for particle in self.weather_particles:
                particle["twinkle"] += particle["speed"] * 0.02

        elif self.weather_type == "scan":
            for scan in self.weather_particles:
                scan["y"] += scan["speed"]
                if scan["y"] > height + 20:
                    scan["y"] = -20

        elif self.weather_type == "horizon":
            for ray in self.weather_particles:
                ray["angle"] += ray["speed"]

        elif self.weather_type == "aurora":
            for band in self.weather_particles:
                band["offset"] += band["speed"]

        elif self.weather_type == "pulse":
            for particle in self.weather_particles:
                particle["phase"] += particle["speed"] * 0.035
    ...
    def draw_weather(self):
        ...
        if self.weather_type == "rain":
            ...
            if self.lightning_flash > 0.05:
                ...
                self.fx_surface.fill((255, 255, 255, alpha))
                self.screen.blit(self.fx_surface, (0, 0))
            ...
            self.fx_surface.fill((*self.world_ambient, 10))
            self.screen.blit(self.fx_surface, (0, 0))
            ...
            for ember in self.weather_particles:
                ...
                glow = (math.sin(t * 3 + ember["sway"]) + 1) * 0.5
                color = tuple(
            ...
            for leaf in self.weather_particles:
                x = leaf["x"] + math.sin(leaf["sway"]) * 13
                y = leaf["y"]
                r = max(2, int(leaf["size"]))

                pygame.draw.ellipse(
                    self.screen,
                    self.world_ambient,
                    (int(x - r), int(y - r * 0.55),
                     int(r * 2), int(r * 1.1))
                )

        elif self.weather_type == "bubbles":
            for bubble in self.weather_particles:
                x = bubble["x"] + math.sin(bubble["sway"]) * 12
                y = bubble["y"]
                r = max(2, int(bubble["size"]))
            ...
            for petal in self.weather_particles:
                x = petal["x"] + math.sin(petal["sway"]) * 12
                y = petal["y"]
                r = max(2, int(petal["size"]))

                pygame.draw.ellipse(
                    self.screen,
                    self.world_ambient,
                    (int(x - r), int(y - r * 0.65),
                     int(r * 2), int(r * 1.3))
                )

        elif self.weather_type == "stars":
            for star in self.weather_particles:
                pulse = (math.sin(star["twinkle"]) + 1) * 0.5
                r = max(1, int(star["size"] + pulse * 1.5))
        ...
        elif self.weather_type == "sparkles":
            for sparkle in self.weather_particles:
                pulse = (math.sin(sparkle["twinkle"]) + 1) * 0.5
                r = max(1, int(sparkle["size"] + pulse * 2))
            ...
            for scan in self.weather_particles:
                y = int(scan["y"])
                alpha = 45 + int((math.sin(t * 2 + y * 0.02) + 1) * 20)

                self.fx_surface.fill((0, 0, 0, 0))
                pygame.draw.line(
                    self.fx_surface,
                    (*self.world_ambient, alpha),
                    (0, y),
                    (width, y),
                    2
                )
            ...
            for i, ray in enumerate(self.weather_particles):
                angle = ray["angle"]
                length = width * 1.25
                spread = 0.045 + i * 0.004
            ...
            for i, band in enumerate(self.weather_particles):
                y = height * (0.12 + i * 0.075)
                wave = math.sin(t * 0.7 + band["offset"]) * 35
                width_band = band["width"]

                pygame.draw.arc(
                    self.fx_surface,
                    (*self.world_ambient, 18),
                    (
                        -width_band + wave,
                        int(y - 55),
                        int(width + width_band * 2),
                        145
                    ),
                    0.15,
                    3.0,
            ...
            pulse = (math.sin(t * 1.7) + 1) * 0.5

            for particle in self.weather_particles:
                local = (math.sin(particle["phase"]) + 1) * 0.5
                r = max(1, int(particle["size"] + local * 5))
                x = int(particle["x"])
                y = int(particle["y"])
            ...
            # Very subtle global pulse.
            if pulse > 0.75:
                self.fx_surface.fill((0, 0, 0, 0))
                alpha = int((pulse - 0.75) * 45)
                self.fx_surface.fill((*self.world_ambient, alpha))
                self.screen.blit(self.fx_surface, (0, 0))
